...
Staff registration creates user with generated default password (emailed); `must_change_password=true` until first change via `/admin/password/change`. When `STAFF_KEYCLOAK_ENABLED=true`, `POST /agents/register` / update / delete and `php artisan manage:admin` push the user to realm `pki-portal` (Admin API, confidential client `backoffice-staff-admin`). Default: **no Keycloak SMTP** — `WelcomeAgentJob` still runs (log/kafka driver) and writes the same password to Keycloak (`reset-password`). Set `KC_STAFF_EXECUTE_ACTIONS_EMAIL=true` only when realm Email is configured; then Laravel skips the job and Keycloak sends `UPDATE_PASSWORD`. `execute-actions-email` 500 without SMTP is expected. Requires `KC_STAFF_ADMIN_SECRET`. Keycloak-only users are still not imported.
